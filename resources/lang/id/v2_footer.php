<?php

return [
    'banner_title' => 'Dominasi Pasar Anda! ',
    'banner_btn' => 'Hubungi Marketing Kami',
    'banner_text' => 'Dengan pengetahuan SEO mendalam, cmlabs berdedikasi membantu brand menjangkau audiens potensial untuk menaklukkan dunia digital mereka',

    'title' => 'Tools tambahan untuk Spesialis SEO & Penulis Konten',
    'sub_title' => 'Ekslusif untuk member cmlabs',
    'paragraph' => 'cmlabs berusaha membantu perusahaan Anda dalam meningkatkan aktivitas SEO. Kami menyebutnya end-to-end SEO melalui Jasa SEO, Jasa Penulisan Konten SEO, Pemasaran Konten, Backlink ke Media Nasional dan Backlink. Selain itu, cmlabs masih memiliki SEO Tools yang dirancang untuk webmaster dan penulis konten untuk memenuhi kebutuhan mereka. Berikut beberapa SEO Tools gratis hanya untuk Anda!',
    'usp_1' => 'Perayapan unlimited di SEO Tools',
    'usp_1_desc' => 'Audit dan optimalkan situs web Anda dengan Alat SEO kami',
    'usp_2' => 'Gratis 180K untuk Keyword Rank Tracker',
    'usp_2_desc' => 'Dapatkan kredit sebesar 180K untuk pengguna baru',
    'usp_3' => 'Akses penuh ke panduan SEO dan istilahnya',
    'usp_3_desc' => 'Pelajari lebih lanjut tentang SEO di cmlabs',

    'mid_title' => 'Level up your <strong>SEO!</strong>',

    'sub_product' => 'Produk SEO',
    'sub_service' => 'Jasa',
    'sub_tool' => 'SEO Tools',
    'sub_resource' => 'Sumber Informasi',
    'sub_company' => 'Perusahaan',
    'sub_partner' => 'Kolaborasi & Aliansi',
    'sub_compliance' => 'Penyesuaian ',
    'sub_info' => 'Informasi',
    'sub_join' => 'Bergabunglah dengan kami!',

    'menu_partner' => 'Jadi Partner cmlabs?',

    'menu_visit-jakarta' => "<b>Berencana mengunjungi Jakarta?</b> Ayo hangout",
    'menu_visit-malang' => '<b>Berencana mengunjungi Malang?</b> Mari bergabung!',

    'menu_serp' => 'Keyword Tracker',
    'menu_service_seo' => 'Jasa SEO',
    'menu_service_writing' => 'Penulisan Konten SEO',
    'menu_service_marketing' => 'Pemasaran Konten',
    'menu_service_backlink' => 'Backlink',
    'menu_service_backlink_national' => 'Backlink ke Media Nasional',

    'menu_tools_webmaster' => 'Untuk Webmaster',
    'menu_tools_writer' => 'Untuk Penulis Artikel',

    'menu_blog' => 'Blog',
    'menu_seo_terms' => 'Kamus SEO',
    'menu_seo_guidelines' => 'Panduan SEO',
    'menu_references' => 'Referensi',

    'menu_about' => 'Tentang kami',
    'menu_contact' => 'Hubungi kami',
    'menu_career' => 'Karir',
    'menu_press' => 'Ruang Press',
    'menu_marketing' => 'Tim Marketing',

    'menu_info' => "Informasi",
    "menu_notif" => "Pusat Notifikasi",
    "menu_promo" => "Promo",
    "menu_partner-2" => "Menjadi Partner",
    "menu_testi" => "Studi Kasus",
    "menu_affiliate" => "Program Afiliasi",

    'menu_tos' => 'Ketentuan Layanan',
    'menu_disclaimer' => 'Disclaimer',
    'menu_privacy' => 'Kebijakan Privasi',
    'menu_tnc' => 'S&K',
    'menu_terms_services' => 'Ketentuan Layanan',

    'menu_sme' => 'SEO untuk Usaha Kecil',
    'menu_enterprise' => 'SEO untuk Perusahaan',
    'menu_pricing' => 'Harga',

    'find_us' => 'Temukan kami',

    // new lang for welcome-page
    "bottom_title" => "Tingkatkan SEO Anda",

    "menu_seo_consultant" => "SEO Consultant",
    "menu_seo_writing" => "SEO Writing",
    "menu_media_buying" => "Media Buying",
    "menu_service_backlink_national_2" => "Backlink ke Media Nasional",
    "menu_press_release" => "Press Rilis",
    "menu_seo_smes" => "SEO untuk UKM",
    "menu_insight" => "Wawasan Umum",

    'sub_partnership' => "Partnership",
    'menu_contributor' => "Program Kontributor",
    'title-2' => 'Tools tambahan untuk Spesialis SEO & Penulis Konten',
    "menu_testimony" => "Testimoni Klien",

    // new lang for welcome-page
    "bottom_title" => "Level Up Your SEO",

    "menu_seo_consultant" => "SEO Consultant",
    "menu_seo_writing" => "SEO Writing",
    "menu_media_buying" => "Media Buying",
    "menu_service_backlink_national_2" => "Backlink for National Media",
    "menu_press_release" => "Press Release",
    "menu_seo_smes" => "SEO for SMEs",
    "menu_testimony" => "Client's Testimony",
    "menu_insight" => "Insight",

    'sub_partnership' => "Partnership",
    'menu_contributor' => "Contributor Program",

    "title-2" => "Tools untuk Spesialis SEO, Penulis & Web Developer",

    'menu_serp' => 'Keyword Tracker',

    'menu_service_seo' => 'Jasa SEO <br><i class="text-primary-70 s-700 b2-m-700">mulai dari Rp10juta</i>',
    'menu_service_writing' => 'Jasa Penulisan Artikel SEO<br><i class="text-primary-70 s-700 b2-m-700">mulai dari Rp450ribu</i>',
    'menu_service_marketing' => 'Media Buying <br><i class="text-primary-70 s-700 b2-m-700">mulai dari Rp100juta</i>',
    'menu_service_expert_writing' => 'Expert Writing <br><i class="text-primary-70 s-700 b2-m-700">mulai dari Rp10.5juta</i>',
    'menu_service_backlink' => 'Backlink dari Blogger',
    'menu_service_political' => 'SEO untuk Kampanye Politik',
    'menu_service_pr' => 'PR / Advertorial',
    'menu_service_cta' => 'Hanya untuk Anda!!<br><i class="s-700 b2-m-700 font-italic">Dapatkan sekarang..</i>',

    "menu_become-partner" => "Menjadi Mitra,<br><i class='text-success-70 s-700 b2-m-700'>untuk Agensi</i>",
    "menu_affiliate" => "Program Afiliasi,<br><i class='text-success-70 s-700 b2-m-700'>untuk Bizdev</i>",
    "menu_contributor" => "Kontributor,<br><i class='text-success-70 s-700 b2-m-700'>untuk Penulis</i>",
    'menu_marketing' => 'Tim Marketing',

    "menu_antigratification" => "Kebijakan Anti-Korupsi & Gratifikasi",
    "menu_generative" => "Panduan Generative-AI",
    "menu_image" => "Jaminan penggunaan gambar komersial",
    "menu_off-page" => "Panduan Aktivitas Optimasi Off-Page",
    "menu_eeat" => "Penilaian E-E-A-T Konten",
    "menu_plagiarism" => "Kebijakan Jaminan Bebas Plagiarisme",
    "menu_local-regulator" => "Kepatuhan Terhadap Regulator",

    "menu_blog" => "cmlabs Blog",
    "menu_terms" => "Kamus SEO",
    "menu_guidelines" => "Panduan SEO",
    "menu_insight" => "Insight",
    "menu_ebook" => "E-Book Gratis",
    "menu_news" => "cmlabs News",
    'menu_class' => 'cmlabs Class<br><i class="text-yellow-80 s-700 b2-m-700">Acara Eksklusif</i>',

    "menu_notif" => "Pusat Pemberitahuan",
    "menu_client" => "Testimoni Klien",
    "menu_faq" => "FAQ Layanan cmlabs",

    "menu_career" => "Karir",
    "menu_benefit" => "Benefit untuk Karyawan",
    "menu_intern" => "Program Magang",
    "menu_fulltime" => "Posisi Fulltime",

    "badge-title" => "Aliansi strategis ini memungkinkan kami menawarkan berbagai solusi SEO inovatif dan layanan yang berkualitas kepada klien.",
    "badge-name" => "Mitra Bisnis Sequence",
    "badge-desc" => "Psst! Hai, perusahaan SEO Stats and Tools! <strong>Jika berencana untuk menguasai pasar Indonesia</strong>, Anda telah datang ke tempat yang tepat! ",
    "badge-desc-cta" => "Mari kita wujudkan rencana itu menjadi kenyataan!",

    "copyright" => "HAK CIPTA"
];
