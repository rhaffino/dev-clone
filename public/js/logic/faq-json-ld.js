...
jQuery('#add').click(function () {
    ...
    print(data);
    jQuery('#form').append(
        "<div class='row parent' data-id='" + (data.mainEntity.length - 1) + "'><div class='col-10 col-sm-11'><div class='form-group mb-5'><label for='question-" + (data.mainEntity.length) + "' data-id='" + (data.mainEntity.length - 1) + "' class='font-weight-bold question'>" + label_question + " " + (data.mainEntity.length) + "</label><textarea class='form-control question' name='' value='' data-id='" + (data.mainEntity.length - 1) + "' placeholder='"+ placeholder_question +"'></textarea></div><div class='form-group mb-5'><label for='answer-" + (data.mainEntity.length) + "' data-id='" + (data.mainEntity.length - 1) + "' class='font-weight-bold answer'>" + label_answer + " " + (data.mainEntity.length) + "</label><textarea class='form-control answer' name='' value='' data-id='" + (data.mainEntity.length - 1) + "' placeholder='"+ placeholder_answer +"'></textarea></div></div><div class='col-2 col-sm-1'><div class='d-flex justify-content-center mt-9'><i class='bx bxs-x-circle bx-md delete' data-id='" + (data.mainEntity.length - 1) + "'></i></div></div></div>"
    );
...
jQuery('#reset').click(function () {
    let data = getDataFromText()
    for (let i = 1; i < data.mainEntity.length; i++) {
        jQuery('.row[data-id=' + i + ']').remove();
    }
    data.mainEntity.splice(1, data.mainEntity.length - 1);
    data.mainEntity[0].name = '';
    data.mainEntity[0].acceptedAnswer.text = '';
    jQuery('.question[data-id=' + 0 + ']').val(main.mainEntity[0].name)
    jQuery('.answer[data-id=' + 0 + ']').val(main.mainEntity[0].acceptedAnswer.text)
